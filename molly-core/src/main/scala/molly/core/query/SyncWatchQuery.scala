...
final case class SyncWatchQuery[F[_], A] private[core] (
 private[core] val iterable: ChangeStreamIterable[BsonDocument]
)(implicit f: Async[F], codec: MollyCodec[F, A]) {
   ...
   /** [[https://mongodb.github.io/mongo-java-driver/5.1/apidocs/mongodb-driver-reactivestreams/com/mongodb/reactivestreams/client/ChangeStreamPublisher.html#fullDocument(com.mongodb.client.model.changestream.FullDocument)]]
     */
   def fullDocument(fullDocument: FullDocument): SyncWatchQuery[F, A] = SyncWatchQuery(
   ...
   private def fromCursor(
    ...
    bufferSize: Int
      ...
      def getNextChunk(cursor: Cursor): F[Option[(Chunk[ChangeStreamDocument[BsonDocument]], Cursor)]] =
         f.suspend(Sync.Type.Blocking) {
            val bldr = Vector.newBuilder[ChangeStreamDocument[BsonDocument]]
            var cnt = 0
            while (cnt < bufferSize && cursor.hasNext) {
               bldr += cursor.next()
               cnt += 1
            }
            if (cnt == 0) None else Some((Chunk.from(bldr.result()), cursor))
         }.cancelable(f.delay(cursor.close()))

      Stream
         .unfoldChunkEval(cursor)(getNextChunk)
         .evalMap(doc => WatchQuery.decodeChangeStreamDocument(doc)(f, codec))
   }
